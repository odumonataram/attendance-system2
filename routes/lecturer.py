...
def dashboard():
    ...
    total_students = sum([course.get_registered_students_count() for course in courses])
...
def courses():
    ...
    return render_template('lecturer/courses.html', courses=lecturer_courses)
...
def course_detail(course_id):
    ...
    students = [User.query.get(reg.student_id) for reg in registrations]
    
    # Get attendance sessions
    sessions = QRSession.query.filter_by(course_id=course_id).order_by(QRSession.generated_at.desc()).all()
...
@lecturer.route('/register-students/<int:course_id>', methods=['GET', 'POST'])
@login_required
@lecturer_required
def register_students(course_id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        flash(f'{registered_count} student(s) registered successfully!', 'success')
    ...
    registered = CourseRegistration.query.filter_by(course_id=course_id).all()
    registered_ids = [reg.student_id for reg in registered]
    
    return render_template('lecturer/register_students.html',
                         course=course,
                         students=all_students,
                         registered_ids=registered_ids)

@lecturer.route('/unregister-student/<int:course_id>/<int:student_id>', methods=['POST'])
@login_required
@lecturer_required
...
@lecturer.route('/generate-qr/<int:course_id>', methods=['GET', 'POST'])
@login_required
@lecturer_required
def generate_qr(course_id):
    ...
    # Verify lecturer owns this course
    if course.lecturer_id != current_user.id:
        flash('You do not have permission to generate QR for this course.', 'danger')
        return redirect(url_for('lecturer.dashboard'))
    
    if request.method == 'POST':
        # Deactivate any previous active sessions for this course
        old_sessions = QRSession.query.filter_by(course_id=course_id, is_active=True).all()
        for session in old_sessions:
            session.deactivate()
        
        # Create new QR session
        qr_session = QRSession.create_session(course_id, expiration_minutes=15)
        
        # Generate QR code URL
        attendance_url = f"http://192.168.1.34:5000{url_for('auth.student_login', token=qr_session.qr_code_data)}"
        
        # Generate QR code image
        qr_image = generate_qr_code(attendance_url)
        
        return render_template('lecturer/display_qr.html',
                             course=course,
                             qr_session=qr_session,
                             qr_image=qr_image,
                             attendance_url=attendance_url)
    
    return render_template('lecturer/generate_qr.html', course=course)
...
def attendance_session(session_id):
    ...
    attendance_records = Attendance.query.filter_by(qr_session_id=session_id).all()
    ...
    all_registrations = CourseRegistration.query.filter_by(course_id=course.id).all()
    total_registered = len(all_registrations)
    
    return render_template('lecturer/attendance_session.html',
                         qr_session=qr_session,
                         course=course,
...
def attendance_live(session_id):
    ...
    # Get attendance records
    attendance_records = Attendance.query.filter_by(qr_session_id=session_id).order_by(Attendance.marked_at.desc()).all()
    ...
    total_registered = CourseRegistration.query.filter_by(course_id=course.id).count()
...
def export_attendance(session_id):
    ...
    for record in attendance_records:
        ...
        data.append({
            ...
            'Email': student.email,
            'Time Marked': record.marked_at.strftime('%Y-%m-%d %I:%M:%S %p')
        })
    ...
    filename = f"attendance_{course.course_code}_{qr_session.generated_at.strftime('%Y%m%d_%H%M%S')}.xlsx"
...
def attendance_history(course_id):
    ...
    for session in sessions:
        attendance_count = Attendance.query.filter_by(qr_session_id=session.id).count()
        session_data.append({
            'session': session,
